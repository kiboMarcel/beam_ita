...
<body>
  ...
  ->where('season_id', $season_id)->first();
    ...
    //exam decision
    $getDecision =  App\Models\Decision::where('year_id', $item->year_id )
        ->where('student_id', $item->student_id)
        ->where('class_id', $item->class_id )
        ->where('group_id', $item->group_id )
        ->where('branch_id', $item->branch_id )->first();

        $examDecision = null;
        if($getDecision != null){
            $examDecision = $getDecision->decision;
        }
  ...
  $countRank =count($annualRank->toArray());

$list = $annualRank->toArray();

for ($i = 0; $i < $countRank; $i++) {
if ($list[$i]['student_id'] == $item->student_id) {
    $annualRank = $list[$i]['annualRank'];
}
}
/* for ($i = 0; $i < count($annual_Rank->toArray()); $i++) {
  if ($annual_Rank[$i]->student_id == $item->student_id) {
      $annualRank = $annual_Rank[$i]->annualRank;
  }
} */
