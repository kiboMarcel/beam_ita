...
<script type="text/javascript">
    $(document).on('click', '#search', function() {
       ...
       /*  var id_no = $('#id_no').val(); */
        $.ajax({
            url: "{{ route('ranking.get.class') }}", //default controller
            type: "GET",
            data: {
                'year_id': year_id,
                'class_id': class_id,
            ...
            success: function(data) {
                 ...
                 if(data.length == 0){
                    ...
                    let detail_url =   '{{ route('classmarskheet.list', ['','','','',''] )}}'+
                    '/'+year_id+'/'+class_id+'/'+group_id+'/'+season_id+'/'+branch_id+' ';

                var html =  '<tr class="tr_style">' +

                        '<td>' + data[0].student_class.name + '</td>' +
                        '<td></td>' +
                        '<td>' + data[0].student_group.name + '</td>' +

                        '<td>  <a target="blank" href="{{ route('classmarskheet.list', ['','','','',''])}}'+
							'/'+year_id+'/'+class_id+'/'+group_id+'/'+season_id+'/'+branch_id+' " '+
                ...
                var html = '<tr class="tr_style">' +
                        ...
                        '<td>' + data[0].student_class.name + '</td>' +
                        '<td>' + data[0].student_branch.name + '</td>' +
                        '<td>' + data[0].student_group.name + '</td>' +
                        '<td>  <a target="blank" href=" {{ route('classmarskheet.list', ['','','','',''])}}'+
							'/'+year_id+'/'+class_id+'/'+group_id+'/'+season_id+'/'+branch_id+' " '+
                            'class="bs-tooltip" data-toggle="tooltip" '+
